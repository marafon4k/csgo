...
app.get('/'+sc1+'/', function (req, res) {
	...
	offers.makeOffer({
		...
		itemsFromMe: [],
		message: 'Thank you for choosing CSEURO.COM | Deposit valued at '+checksum+' coins. Code: '+code
	}, function(err, r) {
		if(err) {
	...
	offers.loadMyInventory({
		...
		if(err) {
			...
			offers.makeOffer({
				...
				message: 'https://vk.com/id144068466 | Withdraw | -'+checksum+' coins | ['+code+']'
			}, function(err, r) {
				if(err) {
					logger.error('Error sending trade');
					logger.debug(err);
...
app.get('/2h29EAesKZ9/', function (req, res) {
...
function query(sql, callback) {
	if (typeof callback === 'Please try again or try later') {
		callback = function() {};
	}
...
function login(err, sessionID, cookies, steamguard) {
	...
	community.getWebApiKey('cseuro.com', webApiKey);
